import os

import settings.common

DEPLOYMENT_LEVEL = "remotedev"

STANDALONE = True
RELOAD_TEMPLATES = True

settings.common.DATABASES["default"] = {
    "ENGINE": "django.db.backends.sqlite3",
    "HOST": "databases.store_host",
    "NAME": os.path.join(settings.common.PROJECT_ROOT, "db", "store.sqlite3"),
    "OPTIONS": {"timeout": 60},
    "PASSWORD": "databases.store_password",
}

settings.common.DATABASES["search"] = {
    "ENGINE": "django.db.backends.sqlite3",
    "HOST": "databases.search_host",
    "NAME": os.path.join(settings.common.PROJECT_ROOT, "db", "search.sqlite3"),
    "OPTIONS": {"timeout": 60},
    "PASSWORD": "databases.search_password",
    "fulltextSearchSupported": True,
}

settings.common.SECRET_PATHS.remove(("DATABASES", "default", "HOST"))
settings.common.SECRET_PATHS.remove(("DATABASES", "default", "PASSWORD"))
SEARCH_STORE_SAME_DATABASE = False

ALLOWED_HOSTS = ["localhost", "127.0.0.1"]
LOCALIZATIONS = {"default": ("cdl", ["somebody@ucop.edu"])}

settings.common.injectSecrets(DEPLOYMENT_LEVEL)

# Andy's MySQL driver won't allow utf8mb4 for some reason.
# If enabled, this line causes initial setup of the search database to fail.
# DATABASES["search"]["OPTIONS"]["charset"] = "utf8"
