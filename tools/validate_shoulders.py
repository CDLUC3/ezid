...
#     import shoulder_parser
#   except ImportError:
#     sys.path.append(os.path.join(os.path.split(thisDir)[0], "impl"))
#     import shoulder_parser
...
p.add_option(
    "-m",
    action="store",
    dest="minterCredentials",
    default=None,
    metavar="USERNAME:PASSWORD",
    help="minter server credentials",
)
options, args = p.parse_args()
if len(args) < 1:
    p.print_help()
    sys.exit(1)
if options.minterCredentials is not None:
...
entries, errors, warnings = impl.shoulder_parser.parse(fileContent)
...
def testMinter(shoulder, minter):
    try:
        id = impl.noid_nog_standalone.Minter(
            minter, *options.minterCredentials
        ).mintIdentifier()
    except Exception as e:
    ...
    if shoulder.startswith("ark:/"):
        ...
        id = "doi:" + impl.util.shadow2doi(id)
    if not id.startswith(shoulder):
        error(
            "shoulder '%s': minted identifier (%s) does not match shoulder"
            % (shoulder, id)
        )
...
def testDatacenter(shoulder, datacenter):
    c = None
    try:
        c = urllib.request.urlopen("https://api.datacite.org/clients/" + datacenter)
        prefixes = [
            p["id"]
            for p in json.loads(c.read())["data"]["relationships"]["prefixes"]["data"]
        ]
    except Exception as e:
        error(
            (
                "shoulder '%s': error loading data for datacenter %s: "
                + "DataCite API error: %s"
            )
            % (shoulder, datacenter, str(e))
        )
        return
    finally:
        if c is not None:
