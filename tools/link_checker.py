...
class OwnerWorkset(object):
    ...
    def isFinished(self):
        ...
        if not self.isLocked and (
            self.owner_id in _permanentExcludes or self.owner_id in _temporaryExcludes
        ):
...
def loadExclusionFile():
    ...
    if _exclusionFile is None:
        return
    if nowi() - _lastExclusionFileCheckTime < 10:
        return
    ...
    try:
        ...
        if s.st_mtime == _exclusionFileModifyTime:
            return
        # noinspection PyTypeChecker
        ...
        for l in f:
            ...
            try:
                (pe if flag == "permanent" else te).append(
                    ezidapp.models.search_user.SearchUser.objects.get(username=user).id
                )
            except ezidapp.models.search_user.SearchUser.DoesNotExist:
...
def updateDatabaseTable():
    ...
    bad = [0, 0, nowi()]
    # noinspection PyTypeChecker
    lcGenerator = harvest(ezidapp.models.link_checker.LinkChecker)
    # noinspection PyTypeChecker
    siGenerator = harvest(
        ...
        and si.owner_id not in _permanentExcludes,
    )
    lc = next(lcGenerator)
    si = next(siGenerator)
    while lc is not None or si is not None:
        if lc is not None and (si is None or lc.identifier < si.identifier):
            ...
            nlc = ezidapp.models.link_checker.LinkChecker(
                identifier=si.identifier, target=si.target, owner_id=si.owner_id
            )
            ...
            if lc.owner_id != si.owner_id or lc.target != si.target:
                ...
                if lc.isUnvisited:
                    ...
                    if lc.isGood:
                        ...
                        bad[0] += 1
                        # noinspection PyUnresolvedReferences
                        if lc.lastCheckTime < nowi() - BAD_RECHECK_MIN_INTERVAL:
                            bad[1] += 1
                        # noinspection PyUnresolvedReferences
...
def loadWorkset():
    ...
    bad = [0, nowi()]
    for user in ezidapp.models.search_user.SearchUser.objects.all().only("id"):
        if user.id in _permanentExcludes or user.id in _temporaryExcludes:
            continue
        ...
        if WORKSET_OWNER_MAX_LINKS - len(qs) > 0:
            q = query(
                False,
                nowi() - GOOD_RECHECK_MIN_INTERVAL,
        ...
        if len(qs) > 0:
            ...
            if len(qs) == WORKSET_OWNER_MAX_LINKS:
                numOwnersCapped += 1
    ...
    try:
        ...
        while True:
            ...
            if not ow.isFinished():
                if (
                    not ow.isLocked
                    and ow.lastCheckTime < t - OWNER_REVISIT_MIN_INTERVAL
                ):
                    ow.isLocked = True
        ...
        while firstRound or remaining(start, TABLE_UPDATE_CYCLE) > 0:
            ...
            if len(_workset) > 0:
                ...
                if firstRound:
                    ...
                    # 1 second.
                    timeout = min(
                        remaining(start, TABLE_UPDATE_CYCLE),
                ...
                _log.info(
                    "end processing, checked %d link%s at %s, slept %s"
