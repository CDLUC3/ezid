...
def process1(args, record):
    ...
    if args.operation == "mint":
        ...
        del record["_id"]
        r = urllib.request.Request(
            "https://ezid.cdlib.org/id_str/" + urllib.parse.quote(id_str, ":/")
        )
    ...
    if args.cookie is not None:
        ...
        r.add_header("Authorization", util.basic_auth(args.username, args.password))
    c = None
    try:
        ...
        if e.fp is not None:
            ...
            return id_str, "error: %d %s" % (e.code, str(e))
    except Exception as e:
