...
class Command(django.core.management.BaseCommand):
    ...
    def list_bdb(self):
        root_path = impl.nog_bdb.bdb.get_bdb_root(self.opt.root_path)
        ...
        for i, (naan_prefix_str, shoulder_str, minter_bdb) in enumerate(
            impl.nog_bdb.bdb.iter_bdb(self.opt.root_path)
        ):
            log.info(
    ...
    def dump(self):
        ...
        impl.nog_bdb.bdb.dump(self.bdb_path)

    def dump_full(self):
        self._assert_bdb_path(exists=True)
        print("Dumping arbitrary BerkeleyDB to HJSON", file=sys.stderr)
        impl.nog_bdb.bdb.dump_full(self.bdb_path)

    def mint(self):
        self._assert_bdb_path(exists=True)
        for i, id_str in enumerate(
            impl.nog_bdb.minter.mint_by_bdb_path(
                self.bdb_path,
                self.opt.count,
    ...
    def forward(self):
        ...
        for i, (naan_prefix_str, shoulder_str, minter_bdb) in enumerate(
            ...
            id_str = '<start>'
            for id_str in impl.nog_bdb.minter.mint_by_bdb_path(
                bdb_path,
                self.opt.count,
                dry_run=False,
            ):
    ...
    def create(self):
        ...
        bdb_path = impl.nog_bdb.minter.create_minter_database(self.opt.ns_str, self.opt.root_path)
    ...
    def slice(self):
        ...
        try:
            bdb_path = impl.nog_bdb.minter.create_minter_database(self.opt.ns_str, dir_path.as_posix())
            for i, id_str in enumerate(
                impl.nog_bdb.minter.mint_by_bdb_path(
                    bdb_path,
                    self.opt.count,
    ...
    def _get_dbd_path(self, is_new):
        try:
            ns = impl.nog_sql.id_ns.IdNamespace.from_str(self.opt.ns_str)
        except impl.nog_sql.exc.MinterError:
            log.info('Argument is not a DOI or ARK. Using it as path: {}'.format(self.opt.ns_str))
            return pathlib.Path(self.opt.ns_str)
        else:
            p = impl.nog_bdb.bdb.get_path(ns, self.opt.root_path, is_new)
    ...
    def _copy_file(self, src_path, dst_path):
        impl.nog_sql.filesystem.create_missing_directories_for_file(dst_path)
