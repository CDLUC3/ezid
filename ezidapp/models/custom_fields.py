...
class StoreIdentifierObjectField (django.db.models.BinaryField):
    ...
    if value is None:
      ...
      try:
        if type(value) in [str, buffer]:
          ...
          v = zlib.compress(django.core.serializers.serialize("json", [value]))
  ...
  def from_db_value (self, value, *args, **kwargs):
    if value is None:
      ...
      try:
        si = django.core.serializers.deserialize("json",
          zlib.decompress(value)).next().object
        ...
        if si.owner_id != None: si.owner = store_user.getById(si.owner_id)
