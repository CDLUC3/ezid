    ...
    with open(file) as f:
        ...
        for l in f:
            ...
            try:
                ...
                assert False, f"{file}, line {n:d}: {str(e)}"
    return m
        ...
        try:
            if m.group(0) == "$$":
                ...
                if ":" in m.group(0):
                    ...
                    try:
                        r = getattr(django.db.models.functions, f)(*args)
                        if isinstance(r, str):
                            v += r
                        else:
                        ...
                        assert (
                            False
                        ), f"error calling user-supplied function {f}: {str(e)}"
                else:
...
def setDataciteValue(node, path, value):
    ...
    if a is not None:
        assert isinstance(
            value, str
        ), f"unsupported interpolation: attribute {a} requires string value"
        n.attrib[a] = value
    else:
...
def transform(args, mappings, row):
    ...
    for i, (d, e) in enumerate(mappings):
        try:
            ...
            assert False, f"{args.mappingsFile}, line {i + 1:d}: {str(err)}"
    if dr is not None:
...
def toAnvl(record):
    ...
    def escape(s, colonToo=False):
        if colonToo:
            ...
            p = "[%\r\n]"
        return re.sub(p, lambda c: f"%{ord(c.group(0)):02X}", s)

    return "".join(
        f"{escape(k, True)}: {escape(record[k])}\n" for k in sorted(record.keys())
    )
...
def process1(args, record):
    ...
    if args.operation == "mint":
        ...
        del record["_id"]
        r = urllib.request.Request(
            "https://ezid.cdlib.org/id/" + urllib.parse.quote(id_str, ":/")
        )
    ...
    if args.cookie is not None:
        ...
        r.add_header(
            "Authorization",
            "Basic "
            + base64.b64encode(args.username + ":" + args.password).decode('utf-8'),
        )
    c = None
    try:
        ...
        if e.fp is not None:
            ...
            return id_str, f"error: {e.code:d} {str(e)}"
    except Exception as e:
        return id_str, "error: " + str(e)
    finally:
        if c is not None:
            c.close()


def formOutputRow(args, row, record, recordNum, id_str, error):
    # row: [value1, value2, ...]
    # record: metadata dictionary
    ...
    for row in csv.reader(
        ...
        try:
            ...
            if args.previewMode:
                ...
                sys.stdout.buffer.write(toAnvl(record).encode("utf-8"))
            else:
                id_str, error = process1(args, record)
                w.writerow(
                    [
                        c.encode("utf-8")
                        for c in formOutputRow(args, row, record, n, id_str, error)
                    ]
                )
                sys.stdout.flush()
        except Exception as e:
            assert False, f"record {n:d}: {str(e)}"
