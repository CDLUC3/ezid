...
_geojsonTypes = ["Point", "MultiPoint", "LineString", "MultiLineString",
  "Polygon", "MultiPolygon", "GeometryCollection"]

def _isNestedList (o, n):
  if type(o) is not list: return False
  if n > 1:
    if len(o) == 0: return False
    if not all(_isNestedList(m, n-1) for m in o): return False
  return True
...
def geojsonPolygonToDatacite (geojson):
  ...
  if type(root) is not dict or\
    root.get("type", "unknown") not in _geojsonTypes:
    return "not a GeoJSON document"
  if len(objects) > 1: return "document defines more than one geometry"
  if len(objects) == 0 or\
    objects[0]["type"] not in ["Polygon", "MultiPolygon"]:
    return "no polygon found"
  p = objects[0]
  if not _isNestedList(p.get("coordinates", {}),
    3 if p["type"] == "Polygon" else 4):
  ...
  for ct in p["coordinates"][0]:
    if len(ct) not in [2, 3] or not isinstance(ct[0], (int, float)) or\
      not isinstance(ct[1], (int, float)) or\
      (len(ct) == 3 and not isinstance(ct[2], (int, float))):
      return "malformed coordinates"
    ...
    if len(ct) == 3 and float(ct[2]) != 0: altitudeWarning = True
  if len(output) < 4:
    return "polygon has insufficient coordinate tuples (4 required)"
  if float(output[0][0].text) != float(output[-1][0].text) or\
    float(output[0][1].text) != float(output[-1][1].text):
