...
class CreatorForm(forms.Form):
  ...
  def __init__(self, *args, **kwargs):
    # Probably not needed. ToDo: Cleanup
    # self.extraFields = kwargs.pop('extraFields')
    ...
    # Min 2-n # of NameIdentifier fields are generated by NameIdMultBaseFormSet
    ...
    # self.fields["nameIdCount"] = forms.CharField(required=False, widget=forms.HiddenInput())
    # for i, a in enumerate(self.extraFields):
    #  self.fields['custom_%s' % i] = forms.CharField(required=False, label=a)
  def clean(self):
...
def _getNameIdCt(fields, prefix):
  """ Tally number of nameIdentifier fields per form
      ...
      Returns array of integers representing count for each form 0 - n
  """
  r = [(0,1)]  # Default 2 nameIds only when first creating an ID
  d = {} 
  if fields:
