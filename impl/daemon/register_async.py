...
# class _StateHolder(impl.daemon.daemon_base.DaemonBase):
class _StateHolder:
    def __init__(
        self,
        registrar,
        ...
        super(_StateHolder, self).__init__()
                ...
                try:
                    ...
                    with django.db.transaction.atomic():
                        for r in rows:
                            r.save()
                    impl.log.otherError(
                        "register_async._workerThread/" + sh.registrar, e
                    )
                else:
                    _checkAbort(sh)
                    with django.db.transaction.atomic():
