...
class CrossRefDaemon(impl.daemon.daemon_base.DaemonBase):
    def __init__(self):
        super(CrossRefDaemon, self).__init__()
    ...
    def validateBody(self, body):
        ...
        m = self._prologRE.match(body)
        if m:
            assert m.group(1) == "1.0", "unsupported XML version"
            if m.group(2) is not None:
                assert self._utf8RE.match(m.group(3)), "XML encoding must be utf-8"
        ...
        assert (
            len(resource) == 1
        ), f"<doi_data> element contains {self._notOne(len(resource))} <resource> subelement"
        resource = resource[0]
        resource.text = "(:tba)"
        assert (
    ...
    def _buildDeposit(
        self, body, registrant, doi, targetUrl, withdrawTitles=False, bodyOnly=False
    ):
    ...
    def replaceTbas(self, body, doi, targetUrl):
        ...
        """
        return self._buildDeposit(body, None, doi, targetUrl, bodyOnly=True)
    ...
    def _pollDepositStatus(self, batchId, doi):
        ...
        try:
            ...
            if root.attrib["status"] != "completed":
                ...
                assert (
                    "status" in d.attrib
                ), "missing record_diagnostic/status attribute"
    ...
    def enqueueIdentifier(self, identifier, operation, metadata, blob):
    ...
    def getQueueStatistics(self):
        ...
        registration failed)."""
        q = ezidapp.models.crossref_queue.CrossrefQueue.objects.values(
            "status"
    ...
    def _queue(self):
        self._checkAbort()
        return ezidapp.models.crossref_queue.CrossrefQueue

    def _doDeposit(self, r):
        m = impl.util.deblobify(r.metadata)
        if r.operation == ezidapp.models.crossref_queue.CrossrefQueue.DELETE:
            url = "http://datacite.org/invalidDOI"
        else:
            url = m["_t"]
        submission, body, batchId = self._buildDeposit(
            m["crossref"],
            ezidapp.models.store_user.getUserByPid(r.owner).username,
            r.identifier[4:],
            url,
            withdrawTitles=(
                r.operation == ezidapp.models.crossref_queue.CrossrefQueue.DELETE
        ...
        if self._submitDeposit(submission, batchId, r.identifier[4:]):
            if r.operation == ezidapp.models.crossref_queue.CrossrefQueue.DELETE:
                ...
                r.status = ezidapp.models.crossref_queue.CrossrefQueue.SUBMITTED
                r.batchId = batchId
    ...
    def _sendEmail(self, emailAddress, r):
        if r.status == ezidapp.models.crossref_queue.CrossrefQueue.WARNING:
            s = "warning"
        else:
            s = "error"
        ...
        if t[0] == "submitted":
            ...
            if r.operation != ezidapp.models.crossref_queue.CrossrefQueue.DELETE:
                if t[0] == "completed successfully":
                    crs = ezidapp.models.store_identifier.StoreIdentifier.CR_SUCCESS
                    crm = ""
                else:
                    if t[0] == "completed with warning":
                        crs = ezidapp.models.store_identifier.StoreIdentifier.CR_WARNING
                    else:
                        crs = ezidapp.models.store_identifier.StoreIdentifier.CR_FAILURE
            ...
            if t[0] == "completed successfully":
                ...
                if t[0] == "completed with warning":
                    r.status = ezidapp.models.crossref_queue.CrossrefQueue.WARNING
                else:
                    r.status = ezidapp.models.crossref_queue.CrossrefQueue.FAILURE
                r.message = t[1]
                u = ezidapp.models.store_user.getUserByPid(r.owner)
                if u.crossrefEmail != "":
                    self._checkAbort()
        ...
        while True:
            ...
            try:
                ...
                if maxSeq is not None:
                    if (
                        self._queue().objects.aggregate(django.db.models.Max("seq"))[
                            "seq__max"
                        ]
                        == maxSeq
                    ):
                ...
                for r in query:
                    ...
                    if (
                        self._queue().objects.filter(identifier=r.identifier).count()
                        > 1
                    ):
                        r.delete()
                        maxSeq = None
                        ...
                        if (
                            r.status
                            == ezidapp.models.crossref_queue.CrossrefQueue.UNSUBMITTED
                        ):
                            self._doDeposit(r)
                            maxSeq = None
                        elif (
                            r.status
                            == ezidapp.models.crossref_queue.CrossrefQueue.SUBMITTED
                        ):
                            self._doPoll(r)
                            maxSeq = None
                        else:
                ...
                impl.log.otherError("crossref._daemonThread", e)
                maxSeq = None

    _prologRE = re.compile(
        r'<\?xml\s+version\s*=\s*[\'"]([-\w.:]+)["\']'
        r'(\s+encoding\s*=\s*[\'"]([-\w.]+)["\'])?'
        r'(\s+standalone\s*=\s*[\'"](yes|no)["\'])?\s*\?>\s*'
    )
    _utf8RE = re.compile("UTF-?8$", re.I)
    _schemaLocation = "{http://www.w3.org/2001/XMLSchema-instance}schemaLocation"
    _schemaLocationTemplate = "http://www.crossref.org/schema/deposit/crossref{}.xsd"
    _tagRE = re.compile(r'{(http://www\.crossref\.org/schema/(4\.[34]\.\d))}([-\w.]+)$')
