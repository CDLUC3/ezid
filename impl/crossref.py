...
def validateBody(body):
  ...
  """
    ...
    assert len(doiData) == 1, "XML document contains %s <doi_data> element" % _notOne(
        len(doiData)
    )
    doiData = doiData[0]
    doi = doiData.findall("N:doi", namespaces=ns)
    assert len(doi) == 1, "<doi_data> element contains %s <doi> subelement" % _notOne(
        len(doi)
    )
    doi = doi[0]
    doi.text = "(:tba)"
    ...
    assert (
        len(resource) == 1
    ), "<doi_data> element contains %s <resource> subelement" % _notOne(len(resource))
    resource = resource[0]
    resource.text = "(:tba)"
    assert (
        doiData.find("N:collection/N:item/N:doi", namespaces=ns) == None
    ), "<doi_data> element contains more than one <doi> subelement"
    e = doiData.find("N:timestamp", namespaces=ns)
    if e != None:
        doiData.remove(e)
    assert (
        doiData.find("N:timestamp", namespaces=ns) == None
    ), "<doi_data> element contains more than one <timestamp> subelement"
    # Normalize schema declarations.
    root.attrib[_schemaLocation] = namespace + " " + (_schemaLocationTemplate % version)
    try:
...
def _wrapException(context, exception):
    ...
    if len(m) > 0:
        m = ": " + m
    return Exception(
        "Crossref error: %s: %s%s" % (context, type(exception).__name__, m)
    )
...
def _submitDeposit(deposit, batchId, doi):
  ...
  """
    ...
    url = _depositUrl % (_testServer if util2.isTestCrossrefDoi(doi) else _realServer)
    try:
        c = None
        try:
            c = urllib2.urlopen(
        ...
        log.otherError(
            ...
            _wrapException(
                "error submitting deposit, doi %s, batch %s" % (doi, batchId), e
            ),
        )
        return False
    else:
...
def _pollDepositStatus(batchId, doi):
  ...
  """
    ...
    url = _resultsUrl % (_testServer if util2.isTestCrossrefDoi(doi) else _realServer)
    try:
        c = None
        try:
            c = urllib2.urlopen(
        ...
        if root.attrib["status"] != "completed":
            ...
            assert len(d) == 1, (
                "<doi_batch_diagnostic> element contains %s "
                + "<record_diagnostic> element"
            ) % _notOne(len(d))
            d = d[0]
            ...
            if d.attrib["status"] == "Success":
                ...
                m = d.findall("msg")
                assert len(m) == 1, (
                    "<record_diagnostic> element contains %s " + "<msg> element"
                ) % _notOne(len(m))
                m = m[0].text
        ...
        log.otherError(
            ...
            _wrapException(
                "error polling deposit status, doi %s, batch %s" % (doi, batchId), e
            ),
        )
        return ("unknown", None)
...
def enqueueIdentifier(identifier, operation, metadata, blob):
  ...
  """
    e = ezidapp.models.CrossrefQueue(
...
def getQueueStatistics():
  ...
  """
    q = ezidapp.models.CrossrefQueue.objects.values("status").annotate(
...
def _doDeposit(r):
    ...
    submission, body, batchId = _buildDeposit(
        m["crossref"],
        ezidapp.models.getUserByPid(r.owner).username,
        r.identifier[4:],
        url,
        withdrawTitles=(
            r.operation == ezidapp.models.CrossrefQueue.DELETE
...
def _sendEmail(emailAddress, r):
    ...
    m = (
        "EZID received a%s %s in registering an identifier of yours with "
        + "Crossref.\n\n"
        + "Identifier: %s\n\n"
        + "Status: %s\n\n"
        + "Crossref message: %s\n\n"
...
def _doPoll(r):
    ...
    if t[0] == "submitted":
        ...
        if r.operation != ezidapp.models.CrossrefQueue.DELETE:
            ...
            s = ezid.setMetadata(
                r.identifier,
                ezidapp.models.getAdminUser(),
                {"_crossref": "%s/%s" % (crs, crm)},
