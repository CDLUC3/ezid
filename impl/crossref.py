...
def _submitDeposit(deposit, batchId, doi):
    ...
    url = _depositUrl.format(
        _testServer if impl.util2.isTestCrossrefDoi(doi) else _realServer
    )
    try:
        c = None
        try:
        ...
        impl.log.otherError(
            ...
            _wrapException(f"error submitting deposit, doi {doi}, batch {batchId}", e),
        )
        return False
    else:
        return True
            ...
            if len(d) > 1:
                ...
                for element in d:
                    ...
                    if element.attrib["status"] == "Success":
                        ...
                        m = d.findall("msg")
                        assert len(m) == 1, (
                            f"<record_diagnostic> element contains {_notOne(len(m))} "
                            f"<msg> element"
                        )
                        m = m[0].text
                ...
                assert len(d) == 1, (
                    f"<doi_batch_diagnostic> element contains {_notOne(len(d))} "
...
def _doDeposit(r):
    ...
    if r.operation == ezidapp.models.crossref_queue.CrossrefQueue.DELETE:
        ...
        url = m["_t"]
    submission, body, batchId = _buildDeposit(
        m["crossref"],
...
def _sendEmail(emailAddress, r):
    if r.status == ezidapp.models.crossref_queue.CrossrefQueue.WARNING:
        ...
        s = "error"
    l = f"{_ezidUrl}/id/{urllib.parse.quote(r.identifier, ':/')}"
    m = (
    ...
    while True:
        ...
        try:
            ...
            if maxSeq is not None:
                if (
                    _queue().objects.aggregate(django.db.models.Max("seq"))["seq__max"]
                    == maxSeq
                ):
                    continue
...
loadConfig()
impl.config.registerReloadListener(loadConfig)
