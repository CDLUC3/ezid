...
def _getMetadata(request):
    ...
    if user is not None:
        r = impl.ezid.getMetadata(
            identifier, user, prefixMatch=options.get("prefix_match", False)
        )
    else:
        r = impl.ezid.getMetadata(
            identifier, prefixMatch=options.get("prefix_match", False)
        )
    if type(r) is str:
        if r.startswith("error: forbidden"):
            ...
            # try matching shoulder info
            s = ezidapp.models.shoulder.getLongestShoulderMatch(identifier)
            if s is None:
                return _response(r)
            r = (identifier, {"meta":str(s)})
    s, metadata = r
...
def resolveArkIdentifier(
    ...
    if identifier_info.inflection:
        ...
        '''
        #TODO: refine the metadata output to match that currently generated by N2T
        s, r = impl.ezid.getMetadata(str(identifier_info), prefixMatch=False)
        return _response(s, anvlBody=impl.anvl.format(r))
    msg = {"id": identifier}
    try:
        ...
        # the appropriate URL for the identifier target
